...
POST /api/consultant/voice/session – create OpenAI Realtime ephemeral token (legacy)
POST /api/consultant/voice/call    – WebRTC SDP exchange (GA unified interface)
...
from app.utils.openai_realtime import (
    create_realtime_client_secret,
    exchange_realtime_call,
    is_openai_quota_error,
    is_realtime_sdp_success,
    openai_error_message,
    parse_client_secret_response,
...
def _build_voice_instructions(
    user_id: int, thread_id: str | None
) -> tuple[str, bool]:
    """Return (system instructions, has_doc_context) for voice sessions."""
    instructions = _BASE_SYSTEM_PROMPT
    has_doc_context = False

    if thread_id:
        if not _validate_thread(thread_id, user_id):
            raise PermissionError("Access denied: invalid thread_id")
        db = get_db()
...
def _get_voice_openai_key() -> str:
    """
    OpenAI key for Realtime voice.

    By default prefers OPENAI_API_KEY from .env so local dev matches the key
    you tested with curl. Set OPENAI_REALTIME_USE_ENV_KEY=false to use Admin
    DB key first (same order as _get_openai_key).
    """
    env_key = (os.getenv("OPENAI_API_KEY") or "").strip()
    use_env_first = os.getenv("OPENAI_REALTIME_USE_ENV_KEY", "true").lower() in (
        "1",
        "true",
        "yes",
    )
    if use_env_first and env_key:
        logger.info(
            "Voice OpenAI key source: OPENAI_API_KEY env (prefix=%s…)",
            env_key[:12],
        )
        return env_key
    key = _get_openai_key()
    if key:
        logger.info(
            "Voice OpenAI key source: admin settings (prefix=%s…)",
            key[:12],
        )
    return key
...
def voice_session():
    ...
    try:
        ...
        try:
            instructions, has_doc_context = _build_voice_instructions(
                user_id, thread_id
            )
        except PermissionError as exc:
            return jsonify({"error": str(exc)}), 403

        openai_key = _get_voice_openai_key()
        if not openai_key:
            return jsonify(
                {
                    "error": (
                        "OpenAI API key is not configured. "
                        "Set OPENAI_API_KEY in .env or Admin → Settings."
                    ),
                    "code": "OPENAI_KEY_MISSING",
                }
            ), 503

        # GA Realtime: POST /v1/realtime/client_secrets (replaces beta /sessions)
        resp = create_realtime_client_secret(openai_key, instructions)
        ...
        parsed = parse_client_secret_response(resp.json())
        return jsonify(
            {
                "success": True,
...
def voice_call():
    """
    GA unified WebRTC: browser sends SDP offer; server exchanges with OpenAI
    using the standard API key and returns the SDP answer.
    """
    try:
        user_id = session["user_id"]
        ...
        # Raw SDP body — preserve bytes (do not strip; SDP parsers need exact text)
        if "application/sdp" in content_type:
            sdp = (request.get_data(as_text=True) or "")
        else:
            data = request.get_json(force=True, silent=True) or {}
        ...
        if not sdp.strip() or "v=0" not in sdp:
            return jsonify({"error": "Missing SDP offer", "code": "SDP_MISSING"}), 400

        logger.info("Voice call SDP received: %s bytes", len(sdp.encode("utf-8")))

        try:
            instructions, _has_doc_context = _build_voice_instructions(
                user_id, thread_id
            )
        except PermissionError as exc:
            return jsonify({"error": str(exc)}), 403
        ...
        safety_id = hashlib.sha256(f"user_{user_id}".encode()).hexdigest()
        resp = exchange_realtime_call(
            openai_key,
            sdp,
            instructions,
        ...
        if not is_realtime_sdp_success(resp):
            detail = openai_error_message(resp)
            logger.error(
                "OpenAI Realtime call failed: %s %s",
                resp.status_code,
                detail[:300],
            )
            if is_openai_quota_error(resp):
                code = "OPENAI_QUOTA_EXCEEDED"
                status = 402
                detail = (
                    "OpenAI Realtime quota exceeded for this API key. "
                    "Add billing/credits at platform.openai.com/settings/organization/billing "
                    "or use a key with Realtime access in .env / Admin settings."
                )
                ...
                status = 502
            payload = {"error": detail, "code": code}
            if resp.status_code == 429 and code == "OPENAI_RATE_LIMIT":
                payload["retry_after_seconds"] = parse_retry_after_seconds(resp)
        ...
        logger.info(
            "OpenAI Realtime call succeeded: status=%s sdp_bytes=%s",
            resp.status_code,
            len(resp.text.encode("utf-8")),
        )
        return Response(resp.text, mimetype="application/sdp")

    except ValueError as exc:
        return jsonify({"error": str(exc), "code": "SDP_INVALID"}), 400
    except requests.exceptions.Timeout:
        return jsonify({"error": "Timeout contacting OpenAI", "code": "TIMEOUT"}), 504
    except Exception as exc:
        logger.error("Consultant /voice/call error: %s", exc, exc_info=True)
        return jsonify({"error": f"Voice call failed: {exc}"}), 500
