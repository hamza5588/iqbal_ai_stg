...
def aggregate_class_topics(class_id: int, teacher_id: int) -> List[dict]:
    ...
    for r in rows:
        bucket = by_topic.setdefault(
            r.topic_id,
            {"topic_id": r.topic_id, "scores": [], "weak_count": 0, "student_count": 0},
        )
        ...
        result.append(
            {
                ...
                "weak_student_count": stats["weak_count"],
...
def get_class_quiz_results(class_id: int, teacher_id: int) -> List[dict]:
    ...
    for a in assignments:
        ...
        for s in submitted:
            if s.attempt_id:
                att = db.query(AssessmentAttempt).filter(AssessmentAttempt.id == s.attempt_id).first()
                if att and att.max_score:
                    scores.append(100.0 * (att.score or 0) / att.max_score)
        results.append(
            {
                "assignment_id": a.id,
                "title": a.title,
        ...
        roster.append(
            {
                ...
                "is_struggling": len(weak) >= 2 or get_overall_progress(enr.student_id) < WEAK_THRESHOLD,
