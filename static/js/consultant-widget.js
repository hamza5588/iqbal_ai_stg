...
(function (global) {
  ...
  /* ── Voice: WebRTC + OpenAI Realtime (GA, server-proxied SDP) ────────── */
  ...
  async function startVoice() {
    if (S.voiceActive || S.voiceStarting) return;
    ...
    statusEl.textContent = 'Starting voice…';
    ...
    try {
      ...
      /* 5 ── SDP offer (wait for ICE candidates) ─────────────────────── */
      ...
      /* 6 ── One server call to OpenAI (unified SDP + session, with retry) */
      statusEl.textContent = 'Connecting to voice AI…';
      var callUrl = '/api/consultant/voice/call';
      if (S.threadId) {
        callUrl += '?thread_id=' + encodeURIComponent(S.threadId);
      }
      var sdpResp = null;
      var maxAttempts = 4;
      for (var attempt = 0; attempt < maxAttempts; attempt++) {
        sdpResp = await fetch(callUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/sdp' },
          body: localSdp,
        });
        if (sdpResp.ok) break;
        if (sdpResp.status === 429 && attempt < maxAttempts - 1) {
          var waitSec = parseInt(sdpResp.headers.get('Retry-After') || '3', 10);
          if (isNaN(waitSec) || waitSec < 1) waitSec = 3;
          statusEl.textContent = 'OpenAI busy — retrying in ' + waitSec + 's…';
      ...
      if (!sdpResp || !sdpResp.ok) {
        var errBody = {};
        try { errBody = await sdpResp.json(); } catch (_) {}
        var errMsg = errBody.error || ('Voice connection failed (' + (sdpResp ? sdpResp.status : 0) + ')');
        if (errBody.code === 'OPENAI_KEY_MISSING') {
          errMsg = 'Voice requires an OpenAI API key. Set OPENAI_API_KEY in .env or Admin → Settings.';
        } else if (errBody.code === 'OPENAI_QUOTA_EXCEEDED') {
          errMsg = errBody.error || 'OpenAI Realtime quota exceeded. Add billing/credits on your OpenAI account or update the API key in .env.';
        } else if (errBody.code === 'OPENAI_RATE_LIMIT') {
          var retryHint = errBody.retry_after_seconds ? (' Wait ' + errBody.retry_after_seconds + 's and try again.') : ' Wait a minute and try again.';
          errMsg = 'OpenAI rate limit reached.' + retryHint;
        }
      ...
      if (err.message === 'Failed to fetch' || (err.name === 'TypeError' && !err.message)) {
        msg = 'Cannot reach the server. Start the app with: python run.py 5001';
