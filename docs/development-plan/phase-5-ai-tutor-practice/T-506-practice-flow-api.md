...
Practice Flow API. See development plan roadmap.

## Objective

Practice session API: question → submit → hint → retry → next.
...
- `See task — implement as needed for T-506`
...
```
You are a senior full-stack engineer implementing task T-506 for IqbalAI LMS.

Practice session API: question → submit → hint → retry → next.

BEFORE CODING:
1. Read docs/development-plan/README.md and dependency tasks listed for T-506
